...
async function fetchAndDisplayStats(barberId, token) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/barber/dashboard-stats`, {
            headers: {
...
async function updateAppointmentStatus(appointmentId, newStatus, barberId, token) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/barber/appointments/${appointmentId}/status`, {
            method: 'PUT',
            headers: {
...
async function saveAvailableDays(barberId, token) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/barber/available-days`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
